...
esp.GetEquippedTool = function(v)
    return (v:FindFirstChildOfClass("Tool") and tostring(v:FindFirstChildOfClass("Tool"))) or "Hands"
end
...
ESP_Loop = rs.RenderStepped:Connect(function()
               for _,i in pairs(game.Workspace:GetChildren()) do
                if i:IsA('Model') and i:FindFirstChild('Backpack') and i:FindFirstChild('Head') then
            local hum = i.Humanoid
            local hrp = i.HumanoidRootPart
            local head = i.Head

            local Vector, onScreen = camera:WorldToViewportPoint(i.HumanoidRootPart.Position)
            ...
            if onScreen and esp.settings_chams.enabled then
                ...
                v.cham.OutlineColor = esp.settings_chams.autocolor and esp.settings_chams.autocolor_outline and esp.WallCheck(i.Head) and esp.settings_chams.visible_Color or esp.settings_chams.autocolor and esp.settings_chams.autocolor_outline and not esp.WallCheck(i.Character.Head) and esp.settings_chams.invisible_Color or esp.settings_chams.outline_color
